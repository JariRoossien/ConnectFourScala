...
class MedAIButton(connectFourGame: ConnectFourGame) extends Button {
    ...
    override def onClick(): Unit = {
        connectFourGame.currentScene = new BoardGameScene(new GameInstance(connectFourGame, true, 4))
        connectFourGame.currentScene.initialize(connectFourGame)

    }

    override val x: Int = 500
    override val y: Int = 310
    ...
    override def getImage(): PImage = connectFourGame.imageProvider.get("medAIButton")
}
